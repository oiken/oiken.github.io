...
class ExcelSearchApp {
  ...
  async handleSearch() {
    ...
    this.showStatus('正在查询中...', 'loading');
    this.clearResults();

    try {
      const files = [
        'excel/汽车类样品-20250221.xlsx',
        'excel/汽车类图纸目录表-20250221.xlsx',
        'excel/消费类样品-20250221.xlsx',
        'excel/IQC工程图纸登记表-20250221.xlsx',
        'excel/环保测试记录表-20250221.xlsx',
      ];
  ...
  formatResults(testResults, sampleResults, drawingResults) {
    ...
    if (sampleResults.length > 0) {
      ...
      // 没有样品编号，没有图纸编号，只有送检记录
      testResults.forEach((testElement) => {
        if (testElement[3].toUpperCase().length > 0) {
          let result = {};
          result.sampleNumber = ''; // 样品编号
          result.partName = testElement[4]; // 规格也就是物料的名字
          result.partNumber = testElement[3]; // 物料编号
          result.supplier = testElement[1]; // 供应商
          result.drawingNumber = ''; // 图纸序号
          result.needTest = '送';
          result.lastTestDate = '';
          result.testIntervalMonths = '';
